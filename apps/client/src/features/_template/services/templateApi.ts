...
export async function fetchTemplateItems(signal?: AbortSignal): Promise<TemplateItem[]> {
  try {
    const res = await fetch(`${API_BASE}/api/template/items`, {
      method: "GET",
    ...
    const json = (await res.json()) as { data?: TemplateItem[] };
    return json.data ?? [];
  } catch (e) {
