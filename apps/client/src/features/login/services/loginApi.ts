...
export async function fetchLoginItems(signal?: AbortSignal): Promise<LoginItem[]> {
  try {
    const res = await fetch(`${API_BASE}/api/login/items`, {
      method: "GET",
    ...
    const json = (await res.json()) as { data?: LoginItem[] };
    return json.data ?? [];
  } catch (e) {
